...
- (nullable NSString *)senderBareWithStream:(XMPPStream *)xmppStream {
    NSString *sender;
    if ([self isGroupChatMessageWithAffiliations]) {
        sender = [self groupChatMessageAffiliationsUser];
    } else if ([self isGroupChatMessage]) {
        if ([[self from] resource]) {
            sender = [[self from] resource];
        } else {
            sender = [[xmppStream myJID] bare];
        }
    } else {
        if ([self from]) {
            NSString *streamBare = [[xmppStream myJID] bare];
            NSString *fromBare = [[self from] bare];
            BOOL outgoing = (streamBare && fromBare && [streamBare isEqualToString:fromBare]);
            sender = (outgoing ? streamBare : fromBare);
        } else {
            sender = [[xmppStream myJID] bare];
        }
    }
...
- (nullable NSString *)conversationBareWithStream:(XMPPStream *)xmppStream {
    ...
    if ([self isGroupChatMessageWithAffiliations]) {
        conversation = [[self from] bare];
    } else if ([self isGroupChatMessage]) {
        if ([[self from] resource]) {
            conversation = [[self from] bare];
        } else {
            conversation = [[self to] bare];
        }
    } else {
        if ([self from]) {
            NSString *streamBare = [[xmppStream myJID] bare];
            NSString *fromBare = [[self from] bare];
            BOOL outgoing = (streamBare && fromBare && [streamBare isEqualToString:fromBare]);
            conversation = (outgoing ? [[self to] bare] : [[self from] bare]);
        } else {
            conversation = [[self to] bare];
        }
    }
